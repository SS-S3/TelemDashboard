...
function computeMissionSummary(telemetry: TelemetryRecord[]): MissionSummary {
  ...
  // Mission duration based on first -> last telemetry timestamps
  ...
  // Packet loss based on Sequence numbers
...
export async function fetchAndProcessData(): Promise<DashboardData> {
  ...
  const parsedTelemetry = Papa.parse<any>(telemetryCsv, {
    header: true,
    dynamicTyping: true,
    skipEmptyLines: true
  });
  const parsedImages = Papa.parse<any>(imagesCsv, {
    header: true,
    dynamicTyping: (field) => field !== 'Timestamp' && field !== 'ImageName',
    skipEmptyLines: true
  });
  ...
  const summary = computeMissionSummary(telemetry);
